...
public class LoginPacket extends cn.nukkit.network.protocol.LoginPacket {
    ...
    private void decodeSkinData() {
        ...
        if (length > 2000000) {
            Server.getInstance().getLogger().warning("[NukkitProtoclPatch] 存在潜在的崩溃包攻击. length="+ length +" - LoginPacket");
            return;
        }
        JsonObject skinToken = decodeToken(new String(this.get(length)));
